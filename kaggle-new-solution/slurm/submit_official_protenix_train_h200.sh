...
RUN_NAME="${RUN_NAME:-protenix_native_chainfix_stage1_$(date +%Y%m%d_%H%M%S)}"
...
SBATCH_EXPORTS=(
  ...
  "NUM_GPUS=${NUM_GPUS}"
  "PROTENIX_NATIVE_TRAIN_MAX_N_TOKEN=${PROTENIX_NATIVE_TRAIN_MAX_N_TOKEN}"
  "PROTENIX_NATIVE_TRAIN_EXCLUDED_MOL_GROUPS=${PROTENIX_NATIVE_TRAIN_EXCLUDED_MOL_GROUPS}"
)
...
sbatch --gpus="${NUM_GPUS}" --export="${EXPORT_STRING}" "$@" "${SLURM_SCRIPT}"
